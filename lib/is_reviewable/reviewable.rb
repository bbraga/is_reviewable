   ...
   module ClassMethods
     ...
     def is_reviewable(*args)
       ...
       options.reverse_merge!(
           :by         => nil,
           :scale      => options[:values] || options[:range] ||
DEFAULTS[:scale],
           :accept_ip  => options[:anonymous] || DEFAULTS[:accept_ip]
# i.e. also accepts unique IPs as reviewer
         )
       scale = options[:scale]
       ...
       options[:total_precision] ||= options[:average_precision] ||
scale.first.to_s.split('.').last.size # == 1

       # Check for incorrect input values, and handle ranges of
floats with help of :step. E.g. :scale => 1.0..5.0.

       if scale.is_a?(::Range) && scale.first.is_a?(::Float)
         options[:step] = (scale.last - scale.first) /
(options[:steps] - 1) if options[:step].blank?
         options[:scale] = scale.first.step(scale.last,
options[:step]).collect { |value| value }
       else
         options[:scale] = scale.to_a.collect! { |v| v.to_f }
       end
       raise InvalidConfigValueError, ":scale/:range/:values must
consist of numeric values only." unless options[:scale].all? { |v|
v.is_a?(::Numeric) }
       raise InvalidConfigValueError, ":total_precision must be an
integer." unless options[:total_precision].is_a?(::Fixnum)

       # Assocations: Review class (e.g. Review).
       options[:review_class] = ASSOCIATION_CLASS

       # Had to do this here - not sure why. Subclassing Review
should be enough? =S
       "::#{options[:review_class]}".constantize.class_eval do
         belongs_to :reviewable, :polymorphic => true unless
self.respond_to?(:reviewable)
         belongs_to :reviewer,   :polymorphic => true unless
self.respond_to?(:reviewer)
       end
       ...
       options[:reviewer_classes] = [*options[:by]].collect do |class_name|
         begin
           ...
           raise InvalidReviewerError, "Reviewer class #{class_name}
not defined, needs to be defined. #{e}"
         end
       end
       ...
       options[:reviewer_classes].each do |reviewer_class|
         if ::Object.const_defined?(reviewer_class.name.to_sym)
           ...
           reviewer_class.class_eval do
             assoc_options = { :through => :reviews, :source =>
:reviewable, :source_type => "#{reviewable.name}" }

             # Handle the case when a Review has a Review. Useful for
'liking' Reviews.
             if reviewable.name == ASSOCIATION_CLASS.name
               assoc_options = {
                 ...
                 :finder_sql => %Q{
                   ...
                   WHERE (("#{reviewable.table_name}".reviewer_id =
#\{id\}) AND ("#{reviewable.table_name}".reviewer_type =
'#{reviewer_class.name}'))
                 }
               }
             end
             ...
             has_many
:"#{reviewable.table_name.singularize}_reviewables", assoc_options
           end

           assoc_options = { :through => :reviews, :source =>
:reviewer, :source_type => "#{reviewer_class.name}" }
           ...
           if reviewable.name == ASSOCIATION_CLASS.name
             assoc_options = {
               ...
               :finder_sql => %Q{
                 ...
                 WHERE (("r".reviewable_id = #\{reviewable_id\}) AND
("r".reviewable_type = '#\{reviewable_type\}'))
           ...
           reviewable.class_eval do
             has_many
:"#{reviewer_class.table_name.singularize}_reviewers", assoc_options
           end
         end
       end
     ...
     protected
       ...
       def validate_reviewer(identifiers)
         raise InvalidReviewerError, "Argument can't be nil: no
reviewer object or IP provided." if identifiers.blank?
         reviewer = identifiers[:by] || identifiers[:reviewer] ||
identifiers[:user] || identifiers[:ip]
         is_ip = Support.is_ip?(reviewer)
         reviewer = reviewer.to_s.strip if is_ip

         unless Support.is_active_record?(reviewer) || is_ip
           raise InvalidReviewerError, "Reviewer is of wrong type:
#{reviewer.inspect}."
         end
         raise InvalidReviewerError, "Reviewing based on IP is
disabled." if is_ip && !self.is_reviewable_options[:accept_ip]
         reviewer
       end
     ...
     def average_rating(recalculate = false)
       if !recalculate && self.reviewable_caching_fields?(:average_rating)
         ...
         ::Review.approved.average(:rating,
           :conditions =>
conditions).to_f.round(self.is_reviewable_options[:total_precision])
       end
     end

     # Calculate average rating for this reviewable object within a
domain of reviewers.
     #
     def average_rating_by(identifiers)
       ...
       ::Review.approved.average(:rating,
           :conditions =>
self.reviewer_conditions(identifiers).merge(self.reviewable_conditions)
     ...
     # * <tt>:reviewer/:user/:account</tt> - identify with a
reviewer-model (e.g. User, ...)
     #
     ...
     def review_by(identifiers)
       self.reviews.find(:first, :conditions =>
reviewer_conditions(identifiers))
     end

     # View the object with and identifier (user or ip) - create new
if new reviewer.
     #
     # === identifiers_and_options hash:
     # * <tt>:reviewer/:user/:account</tt> - identify with a
reviewer-model or IP (e.g. User, Account, ..., "128.0.0.1")
     # * <tt>:rating</tt> - Review rating value, e.g. 3.5, "3.5", ...
(optional)
     # * <tt>:body</tt> - Review text body, e.g. "Lorem *ipsum*..." (optional)
     # * <tt>:*</tt> - Any custom review field, e.g. :reviewer_mood
=> "angry" (optional)
     #
     def review!(identifiers_and_options)
       begin
         ...
         # Except for the reserved fields, any Review-fields should
be be able to update.
         review_values =
identifiers_and_options.except(*::IsReviewable::Review::ASSOCIATIVE_FIELDS)
         review_values[:rating] = review_values[:rating].to_f if
review_values[:rating].present?

         if review_values[:rating].present? &&
!self.valid_rating_value?(review_values[:rating])
           raise InvalidReviewValueError, "Invalid rating value:
#{review_values[:rating]} not in [#{self.rating_scale.join(', ')}]."
         end

         unless review.present?
           # An un-existing reviewer of this reviewable object =>
Create a new review.
           ...
           # An existing reviewer of this reviewable object => Update
the existing review.
         end

         # Update non-association attributes, such as rating, body
(the review text), and any custom fields.
         review.attributes =
review_values.slice(*review.attribute_names.collect { |an| an.to_sym
})
         ...
         raise RecordError, "Could not create/update review
#{review.inspect} by #{reviewer.inspect}: #{e}"
       end
     end
     ...
     def unreview!(identifiers)
       ...
       if review && review.destroy
         ...
         raise RecordError, "Could not un-review #{review.inspect} by
#{reviewer.inspect}: #{e}"
       end
     end
     ...
     protected
       ...
       def update_cache!
         ...
         if self.reviewable_caching_fields?(:average_rating)
           ...
           # self.cached_average_rating = (self.cached_average_rating
+ new_rating) / self.cached_total_reviews.to_f
           self.cached_average_rating = self.average_rating(true)
         end
         self.save(:validate => false) if self.changed?
       end

       # Checks if a certain value is a valid rating value for this
reviewable object.
       #
       ...
       def init_reviewable_caching_fields
         self.cached_total_reviews = 0 if
self.reviewable_caching_fields?(:cached_total_reviews)
         self.cached_average_rating = 0.0 if
self.reviewable_caching_fields?(:average_rating)
       end

       def reviewable_conditions(as_array = false)
         conditions = {:reviewable_id => self.id, :reviewable_type =>
self.class.name}
       ...
       def reviewer_conditions(identifiers, as_array = false)
         ...
         if Support.is_active_record?(reviewer)
           conditions = {:reviewer_id => reviewer.id, :reviewer_type
=> reviewer.class.name}
         else
